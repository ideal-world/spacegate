...
pub struct RedisConfigListener {
...
impl RedisConfigListener {
    pub async fn new(conf_url: &str, check_interval_sec: u64) -> Result<Self, BoxError> {
        ...
        if gateway_configs.is_empty() {
            return Err(BoxError::from(format!("[SG.Config] Gateway Config not found in {CONF_GATEWAY_KEY}")));
        }
        let gateway_configs = gateway_configs.into_values().map(|v| tardis::TardisFuns::json.str_to_obj::<SgGateway>(&v)).collect::<Result<Vec<SgGateway>, _>>()?;
        for gateway_config in gateway_configs {
            ...
            let http_route_configs = http_route_configs
                .into_iter()
                .map(|v| {
                    tardis::TardisFuns::json.str_to_obj::<SgHttpRoute>(&v).map_err(|e| TardisError::format_error(&format!("[SG.Config] Http Route Config parse error {}", e), ""))
                })
                .collect::<TardisResult<Vec<SgHttpRoute>>>()?;
            config.push((gateway_config, http_route_configs));
        }
        let (event_tx, event_rx) = tokio::sync::mpsc::unbounded_channel::<ConfigEvent>();
        for (gateway_config, http_route_configs) in config {
            let _ = event_tx.send(ConfigEvent::GatewayAdd(gateway_config, http_route_configs));
        }
        let handle = tardis::tokio::task::spawn(async move {
                    ...
                    while let Some(changed_key) = key_iter.next_item().await {
                        ...
                        if let Some(gateway_config) = cache_client.hget(CONF_GATEWAY_KEY, changed_gateway_name).await.expect("[SG.Config] cache_client hget failed") {
                            ...
                            match changed_obj {
                                ...
                                "httproute" => {
                                    let _ = event_tx.send(ConfigEvent::HttpRouteReload(gateway_config.name, http_route_configs));
                                }
                                _ => {}
                            }
                        } else {
                            ...
                            if changed_obj == "gateway" {
                                let _ = event_tx.send(ConfigEvent::GatewayDelete(changed_gateway_name.to_string()));
                            }
                        }
                    }
                }
...
impl Stream for RedisConfigListener {
...
impl ConfigListener for RedisConfigListener {
    const CONFIG_LISTENER_NAME: &'static str = "redis";

    fn shutdown(&mut self) {
        info!("[SG.Config] redis listen task shutdown");
        self.handle.abort();
    }
}

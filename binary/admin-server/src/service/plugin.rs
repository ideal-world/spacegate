...
async fn get_wasm_image_schema(Json(req): Json<WasmImageSchemaRequest>) -> Result<Json<Value>, InternalError> {
    ...
    let bytes = match fetch_wasm_image_file_sync_with_auth(image_url, schema_path, req.oci_auth.as_ref()) {
        Ok(bytes) => bytes,
        Err(error) if is_missing_schema_file_error(&error) => return Ok(Json(empty_schema())),
...
fn is_missing_schema_file_error(error: &WasmHostError) -> bool {
    ...
    message.contains("does not contain `") || message.contains("No such file or directory") || message.contains("404 Not Found")
}
    ...
    B: Discovery + Send + Sync + 'static,
{
    Router::new()
        ...
        .route("/wasm/schema", post(get_wasm_image_schema))

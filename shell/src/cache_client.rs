...
use spacegate_kernel::extension::GatewayName;
use tardis::{
...
pub async fn get_from_extension(extensions: &hyper::http::Extensions) -> Option<Arc<TardisCacheClient>> {
    let name = extensions.get::<GatewayName>()?.deref();
    let read = cache_clients().read().await;

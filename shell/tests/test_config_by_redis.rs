...
async fn test_config_by_redis() -> Result<(), BoxError> {
    env::set_var("RUST_LOG", "info,spacegate_shell=trace,spacegate_kernel=trace");
    ...
    // Without keys
    assert!(spacegate_shell::startup_cache(cache_url.clone(), 1).await.is_err());
    ...
    // With cache url
    spacegate_shell::startup_cache(cache_url.clone(), 1).await?;

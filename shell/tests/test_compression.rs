...
async fn test_compression() -> TardisResult<()> {
    env::set_var("RUST_LOG", "info,spacegate_shell=trace,spacegate_plugin=trace,spacegate_kernel=trace");
    tracing_subscriber::fmt::init();
    let server = spacegate_shell::server::RunningSgGateway::create(
        SgGateway {

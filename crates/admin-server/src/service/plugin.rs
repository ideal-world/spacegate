...
async fn sync_attr_cache<B: Discovery>(backend: &B) -> Result<(), BoxError> {
    ...
    if next_sync.elapsed() != Duration::ZERO {
        drop(next_sync);
...
async fn get_list<B: Discovery>(State(AppState { backend, .. }): State<AppState<B>>) -> Result<Json<Vec<String>>, InternalError> {
    sync_attr_cache(backend.as_ref()).await.map_err(InternalError)?;
